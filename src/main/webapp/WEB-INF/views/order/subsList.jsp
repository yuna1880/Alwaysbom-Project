                                ...
                                <li class="d-flex align-items-center border-bottom py-3 bg-light">
                                    <div class="col-7 d-flex align-items-center">
                                        ...
                                        <c:if test="${order.status eq '입금대기' || order.status eq '결제완료'}">
                                            <button data-order-idx="${order.idx}" type="button" class="btn btn-dark btn-sm" id="order-button" onclick="CancelOrder(this, ${status.index})">주문취소</button>
                                        </c:if>
                                        ...
                                        <c:if test="${order.status eq '배송중'}">
                                            <button data-order-idx="${order.idx}" type="button" class="btn btn-dark btn-sm" id="order-button" onclick="ConfirmOrder(this ,${status.index})">구매확정</button>
                                        </c:if>
        ...
        fetch("/admin/api/orders/" + idx + "/status", option).then(response => {
            ...
            response.json().then(result => {
                ...
                if (result) {
                    ...
                    document.querySelector("#order-button").innerText = '배송완료';
                    document.querySelector("#status-text").innerText = '배송완료';
                } else {
                    alert("실패입니다");
                }
            });
